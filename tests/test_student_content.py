...
def test_migrates_legacy_database_without_losing_lessons(tmp_path: Path) -> None:
    ...
    assert repository.applied_migrations() == [
        ...
        (8, "transcript_normalization"),
        (9, "resumable_normalization_chunks"),
        (10, "cloud_processing_privacy"),
    ]
...
def test_migrations_are_idempotent(tmp_path: Path) -> None:
    ...
    assert len(repository.applied_migrations()) == 10
    with repository.connect() as db:
        assert db.execute("SELECT COUNT(*) FROM schema_migrations").fetchone()[0] == 10
