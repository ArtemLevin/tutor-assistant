...
def test_sidebar_groups_pages_and_keeps_tab_indices() -> None:
    ...
    assert navigation.button_for_page(0).text().endswith(
        "Подготовка занятия"
    )
    assert navigation.button_for_page(5).text().endswith("Ученики")
    assert navigation.button_for_page(3).text().endswith(
        "PDF и LaTeX"
    )
...
def test_sidebar_utility_buttons_are_keyboard_accessible() -> None:
    ...
    navigation.command_palette_requested.connect(
        lambda: palette_requests.append(True)
    )

    assert (
        navigation.collapse_button.focusPolicy()
        == Qt.FocusPolicy.StrongFocus
    )
    navigation.collapse_button.setFocus()
    QTest.keyClick(
        navigation.collapse_button,
        Qt.Key.Key_Space,
    )
    ...
    assert (
        navigation.command_button.focusPolicy()
        == Qt.FocusPolicy.StrongFocus
    )
...
class _NavigationWindowHarness:
    def __init__(self) -> None:
        ...
        self.detailed_mode_button = QPushButton(
        ...
        for index in range(8):
            ...
            page_layout.addWidget(
                QLineEdit(f"Поле страницы {index}")
            )
        ...
        self.detailed_mode_button.clicked.connect(
            lambda _checked=False: self._set_mode(
                "detailed"
            )
        )
...
def test_hidden_workspace_tabs_are_restored_and_every_menu_item_opens() -> None:
    ...
    window.navigation_shell = install_information_architecture(
        window
    )
    ...
    assert (
        ...
        is window.navigation_shell
    )
    assert window.tabs.isVisibleTo(
        window.navigation_shell
    )

    for index in range(window.tabs.count()):
        button = (
            window.navigation_shell.button_for_page(
                index
            )
        )
        ...
        assert window.tabs.currentWidget().isVisibleTo(
            window.tabs
        )
...
def test_workspace_and_quick_mode_restore_keyboard_focus() -> None:
    ...
    window.content_stack.show()
    window.navigation_shell = (
        install_information_architecture(window)
    )
    ...
    QTest.keyClick(
        window.navigation_shell.quick_button,
        Qt.Key.Key_Return,
    )
    application.processEvents()
    assert (
        window.content_stack.currentWidget()
        is window.quick_page
    )
    assert (
        QApplication.focusWidget()
        is window.quick_student
    )
