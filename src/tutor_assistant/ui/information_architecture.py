...
class SidebarNavigation(QFrame):
    ...
    def __init__(
        ...
        self.collapse_button.setObjectName(
            "sideNavigationUtility"
        )
        self.collapse_button.setFixedWidth(38)
        self.collapse_button.setAccessibleName(
            "Свернуть боковую навигацию"
        )
        self.collapse_button.setAccessibleDescription(
            "Нажмите Enter или пробел"
        )
        self.collapse_button.setFocusPolicy(
        ...
        for definition in ROUTE_DEFINITIONS:
            ...
            button.navigation_key_pressed.connect(
                lambda key, source=button: self._handle_navigation_key(
                    source,
                    key,
                )
            )
            ...
            self._button_labels[definition.route] = (
                definition.title
            )
            if definition.page_index is None:
                ...
                button.setEnabled(
                    definition.page_index < self.tabs.count()
                )
        ...
        self.command_button.setObjectName(
            "sideNavigationUtility"
        )
        self.command_button.setAccessibleName(
            "Открыть командную палитру"
        )
        self.command_button.setAccessibleDescription(
            "Сочетание клавиш: Ctrl+K"
        )
        self.command_button.setFocusPolicy(
            Qt.FocusPolicy.StrongFocus
        )
        ...
        self.command_button.clicked.connect(
            self.command_palette_requested
        )
    ...
    def navigate(self, route: AppRoute | str) -> bool:
        ...
        if (
            page_index is None
            or not 0 <= page_index < self.tabs.count()
        ):
            return False
        self.tabs.setVisible(True)
        self.tabs.setEnabled(True)
        changed = (
            self.tabs.currentIndex() != page_index
        )
    ...
    def current_route(self) -> AppRoute:
        return (
            route_for_page(self.tabs.currentIndex())
            or AppRoute.TODAY
        )
    ...
    def _handle_navigation_key(
        ...
        if key in {
            ...
            target = enabled_buttons[
                (current + 1) % len(enabled_buttons)
            ]
        elif key in {
            Qt.Key.Key_Up,
            Qt.Key.Key_Left,
    ...
    def set_badges(
        self,
        counts: Mapping[AppRoute | str, int],
    ) -> None:
        self._badges = {
    ...
    def _refresh_button_texts(self) -> None:
        for route, button in self.route_buttons.items():
            ...
            if self._collapsed:
                ...
                button.setText(
                    f"{definition.icon}  "
                    f"{definition.title}{badge}"
                )
                button.setStyleSheet("")
            button.setAccessibleDescription(
                f"{definition.title}. "
                f"Событий, требующих внимания: {count}. "
                f"Сочетание клавиш: {definition.shortcut}"
            )


def install_information_architecture(
    window,
) -> SidebarNavigation:
    tabs = window.tabs
    ...
    def open_quick_mode() -> None:
        ...
        if isinstance(target, QWidget):
            QTimer.singleShot(
                0,
                lambda: target.setFocus(
                    Qt.FocusReason.TabFocusReason
                ),
            )

    navigation.quick_requested.connect(
        open_quick_mode
    )
    detailed_button = getattr(
        window,
...
def _install_stylesheet() -> None:
    ...
    if application.property(
        "ux1InformationArchitectureStyle"
    ):
        return
